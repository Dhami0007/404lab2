...
def send_request(host, port, request):
    ...
    # Create a new socket in with block to ensure it's closed once we are done
...
def handle_connection(conn, addr):
    with conn:
        ...
        while True: # While the client is keeping the socket open
            data = conn.recv(BYTES_TO_READ) # read sine data from the socket
...
def start_server():
    '''
    Create the socket in 'with' block ensure it gets auto-closed once
    we are done
   '''
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        # Bing the server to a specific host and port on this machine
        ...
        # Pass 'conn' off to handdle connection to do some logic
        handle_connection(conn, addr)

# start multithreaded proxy server
def start_threaded_server():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        server_socket.bind((PROXY_SERVER_HOST, PROXY_SERVER_PORT))
        server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR,1)
        server_socket.listen(2) # Allow queuing off up to 2 connectioins
